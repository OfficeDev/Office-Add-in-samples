 ...
 */
async function highlightSelectionHome(event) {
    try {
 ...
 * @param event - The Office add-in command event
 */
async function highlightSelectionCounter(event) {
    try {
...
async function writeValue(event) {
    ...
    try {
        const message = "ExecuteFunction works. Button ID=" + event.source.id;
        await Excel.run(async (context) => {
            const range = context.workbook.getSelectedRange();
...
async function createBubbles(event) {
    ...
    try {
        ...
        if (dotnetloaded) {
            await DotNet.invokeMethodAsync("Blazor.Excel.AddIn.Client", "CreateBubbles");
