...
Running the sample will start two projects, the Blazor server project and the PowerPoint add-in project. The Blazor server project will start a web server that hosts the add-in, and the PowerPoint add-in project will sideload the add-in into PowerPoint.
...
1. Open Visual Studio 2026 and open the **Office-Add-in-samples\Samples\blazor-add-in\Blazor.PowerPoint.AddIn\Blazor.PowerPoint.AddIn.slnx** solution.
1. Ensure both startup projects are configured to **Start**:
   - `Blazor.PowerPoint.AddIn` (server)
   - `Blazor.PowerPoint.AddIn` sideload/npm startup action
1. Choose **Debug** > **Start Debugging** (or press <kbd>F5</kbd>).
   ...
   - The `Blazor.PowerPoint.AddIn` server project starts and runs `npm run start-local` to sideload the add-in and launch PowerPoint.
1. If PowerPoint opens, select the **Sample Add-in** tab and click the buttons to trigger the respective functions.
...
npm --prefix .\Blazor.PowerPoint.AddIn install  
npm --prefix .\Blazor.PowerPoint.AddIn.Client install  

You should see two applications start:
- The Blazor server application running in a console window.
- PowerPoint running the add-in with Sample Add-in tab on the ribbon.

If you for some reason don't see the two applications:
- Right click the Addin Solution
- Select **Configure Startup Projects**
- Set both the Blazor server and PowerPoint add-in to **Start**.

### Optional: run sideload manually from Terminal

For a full manual run, use two terminals.

1. Start the server from `Samples\blazor-add-in\Blazor.PowerPoint.AddIn`:
   - `dotnet run --project .\Blazor.PowerPoint.AddIn\Blazor.PowerPoint.AddIn.csproj`
2. After the server is ready, sideload from `Samples\blazor-add-in\Blazor.PowerPoint.AddIn\Blazor.PowerPoint.AddIn`:
   - `npm run start-local`
...
The TypeScript runs the code to interact with the document and returns.
...
1. Call **JSRuntime.InvokeAsync** to import the JavaScript module and set up the interop.
1. Use **JSModule.InvokeVoidAsync** to call TypeScript functions from your C# code.
1. Call Office JS APIs to interact with the document from TypeScript code.
...
This sample is configured to support debugging both JavaScript and C# files.

1. In the **launchSettings.json** file of the **Blazor.PowerPoint.AddIn** project, make sure all instances of `launchBrowser` are set to `false`.
1. In the **Blazor.PowerPoint.AddIn.csproj.user** file of the **Blazor.PowerPoint.AddIn** project, add the `<BlazorAppUrl>` and `<InspectUri>` elements as shown in the following example XML.

**Note:** The port number in the following XML is 7214. You must change it to the port number specified in the **launchSettings.json** file of the **Blazor.PowerPoint.AddIn** project.
...
```powershell
dotnet restore .\Blazor.PowerPoint.AddIn.slnx
dotnet build .\Blazor.PowerPoint.AddIn.slnx -c Debug
```
...
- Run the server, sideload the add-in, and attach the debugger (recommended):
  ...
  - That configuration runs a composite pre-launch task named `Start: Server & Sideload` which performs the sequence below:
    - Starts the `Run: Server` task as a background process (`dotnet run` for the `Blazor.PowerPoint.AddIn` project).
    - Waits for the server to signal readiness (background problem matcher).
    - Runs the `Sideload: start-local` task (which executes `npm run start-local` in the `Blazor.PowerPoint.AddIn` folder) to sideload the manifest into PowerPoint.
  - After the pre-launch task finishes, VS Code will prompt you to pick the running `.NET` process to attach the debugger to — choose the `Blazor.PowerPoint.AddIn` process.
...
- `Run: Server` (background — runs `dotnet run` for the `Blazor.PowerPoint.AddIn` project)
- `Sideload: start-local` (runs `npm run start-local` in `Blazor.PowerPoint.AddIn`)
...
npm --prefix .\Blazor.PowerPoint.AddIn run start-local
```
...
- Port already in use:
  ...
  - Check which process is using the default port (7214) and stop it if safe.

  ```powershell
  netstat -ano | Select-String 7214
