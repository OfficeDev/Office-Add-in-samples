...
1. In any spreadsheet cell, enter the formula `=CONTOSO.SETVALUEFORKEYCF("2","oranges")`. The formula returns the text `Stored key/value pair`.
1. In the task pane, enter the key from the previous formula `2` and select `Get value for key`. The task pane displays the value `oranges`.
...
1. To reroute to localhost, run `office-addin-https-reverse-proxy`. If you didn't install this tool yet, install it by running the following command:
