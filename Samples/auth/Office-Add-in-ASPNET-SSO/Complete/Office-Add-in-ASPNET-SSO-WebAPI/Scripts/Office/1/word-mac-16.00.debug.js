...
OSF.OUtil=(function () {
	...
	return {
		...
		encodeBase64: function OSF_Outil$encodeBase64(input) {
			if (!input)
				return input;
			var codex="ABCDEFGHIJKLMNOP"+"QRSTUVWXYZabcdef"+"ghijklmnopqrstuv"+"wxyz01234544355+/=";
			var output=[];
			var temp=[];
			var index=0;
			var c1, c2, c3, a, b, c;
...
(function (OfficeExtension) {
	var ActionFactory=(function () {
		...
		ActionFactory.createSetPropertyAction=function (context, parent, propertyName, value) {
			...
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.SetProperty,
				Name: propertyName,
		...
		ActionFactory.createMethodAction=function (context, parent, methodName, operationType, args) {
			...
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.Method,
				Name: methodName,
				ObjectPathId: parent._objectPath.objectPathInfo.Id,
				ArgumentInfo: {}
			};
			var referencedArgumentObjectPaths=OfficeExtension.Utility.setMethodArguments(context, actionInfo.ArgumentInfo, args);
			OfficeExtension.Utility.validateReferencedObjectPaths(referencedArgumentObjectPaths);
			var isWriteOperation=operationType !=OfficeExtension.OperationType.Read;
		...
		ActionFactory.createQueryAction=function (context, parent, queryOption) {
			...
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.Query,
				Name: "",
				ObjectPathId: parent._objectPath.objectPathInfo.Id,
			};
		...
		ActionFactory.createRecursiveQueryAction=function (context, parent, query) {
			...
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.RecursiveQuery,
				Name: "",
				ObjectPathId: parent._objectPath.objectPathInfo.Id,
		...
		ActionFactory.createInstantiateAction=function (context, obj) {
			...
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.Instantiate,
				Name: "",
		...
		ActionFactory.createTraceAction=function (context, message, addTraceMessage) {
			var actionInfo={
				Id: context._nextId(),
				ActionType: OfficeExtension.ActionType.Trace,
				Name: "Trace",
				ObjectPathId: 0
			};
			var ret=new OfficeExtension.Action(actionInfo, false);
...
(function (OfficeExtension) {
	var ClientRequest=(function () {
		function ClientRequest(context) {
			...
			this.m_flags=OfficeExtension.ClientRequestFlags.None;
			this.m_traceInfos={};
		...
		ClientRequest.prototype.addAction=function (action) {
			if (action.isWriteOperation) {
				this.m_flags=this.m_flags | OfficeExtension.ClientRequestFlags.WriteOperation;
			}
		...
		ClientRequest.prototype.addReferencedObjectPath=function (objectPath) {
			...
			while (objectPath) {
				...
				if (objectPath.objectPathInfo.ObjectPathType==OfficeExtension.ObjectPathType.Method) {
	...
	var ClientRequestContext=(function () {
		function ClientRequestContext(url) {
			...
			if (OfficeExtension.Utility.isNullOrUndefined(url) || typeof (url)==="string" && url.length===0) {
				...
				if (defaultUrlAndHeaders &&
					!OfficeExtension.Utility.isNullOrEmptyString(defaultUrlAndHeaders.url)) {
		...
		ClientRequestContext._runBatch=function (functionName, receivedRunArgs, ctxInitializer, numCleanupAttempts, retryDelay, onCleanupSuccess, onCleanupFailure) {
			...
			if (receivedRunArgs.length > 0 &&
				typeof (receivedRunArgs[0])==="object" &&
				receivedRunArgs[0] !==null &&
				Object.getPrototypeOf(receivedRunArgs[0])===Object.getPrototypeOf({}) &&
				!OfficeExtension.Utility.isNullOrUndefined(receivedRunArgs[0].url)) {
		...
		ClientRequestContext._runCommon=function (functionName, requestInfo, ctxRetriever, batch, numCleanupAttempts, retryDelay, onCleanupSuccess, onCleanupFailure) {
			var starterPromise=new OfficeExtension.Promise(function (resolve, reject) { resolve(); });
			var ctx;
			var succeeded=false;
			var resultOrError;
			return starterPromise
				...
				if (requestInfo &&
					!OfficeExtension.Utility.isNullOrEmptyString(requestInfo.url) &&
					requestInfo.url !==ctx.m_url) {
...
(function (OfficeExtension) {
	...
	var ClientResult=(function () {
		...
		ClientResult.prototype._handleResult=function (value) {
			...
			if (this.m_type===ClientResultProcessingType.date) {
...
(function (OfficeExtension) {
	var EventHandlers=(function () {
		...
		EventHandlers.prototype.add=function (handler) {
			...
			this.m_context._pendingRequest._addPendingEventHandlerAction(this, { id: action.actionInfo.Id, handler: handler, operation: OfficeExtension._Internal.EventHandlerActionType.add });
			return new OfficeExtension.EventHandlerResult(this.m_context, this, handler);
		};
		EventHandlers.prototype.remove=function (handler) {
			var action=OfficeExtension.ActionFactory.createTraceAction(this.m_context, null, false);
			this.m_context._pendingRequest._addPendingEventHandlerAction(this, { id: action.actionInfo.Id, handler: handler, operation: OfficeExtension._Internal.EventHandlerActionType.remove });
		};
		EventHandlers.prototype.removeAll=function () {
			var action=OfficeExtension.ActionFactory.createTraceAction(this.m_context, null, false);
			this.m_context._pendingRequest._addPendingEventHandlerAction(this, { id: action.actionInfo.Id, handler: null, operation: OfficeExtension._Internal.EventHandlerActionType.removeAll });
		};
			...
			for (var i=0; i < actions.length; i++) {
				if (req._responseTraceIds[actions[i].id]) {
					...
					switch (actions[i].operation) {
						case OfficeExtension._Internal.EventHandlerActionType.add:
							handlersResult.push(actions[i].handler);
							break;
						case OfficeExtension._Internal.EventHandlerActionType.remove:
							...
							break;
						case OfficeExtension._Internal.EventHandlerActionType.removeAll:
							handlersResult=[];
							break;
					}
				}
...
(function (OfficeExtension) {
	var ObjectPath=(function () {
		...
		ObjectPath.prototype._updateAsNullObject=function () {
			...
			this.m_isValid=true;
			this.m_objectPathInfo.ObjectPathType=OfficeExtension.ObjectPathType.NullObject;
		...
		ObjectPath.prototype.updateUsingObjectData=function (value) {
			...
			if (!OfficeExtension.Utility.isNullOrEmptyString(referenceId)) {
				this.m_isInvalidAfterRequest=false;
				this.m_isValid=true;
				this.m_objectPathInfo.ObjectPathType=OfficeExtension.ObjectPathType.ReferenceId;
			...
			if (parentIsCollection || !OfficeExtension.Utility.isNullOrEmptyString(getByIdMethodName)) {
				...
				if (!OfficeExtension.Utility.isNullOrUndefined(id)) {
					...
					if (parentIsCollection) {
						this.m_objectPathInfo.ObjectPathType=OfficeExtension.ObjectPathType.Indexer;
						this.m_objectPathInfo.Name="";
					}
					else {
						this.m_objectPathInfo.ObjectPathType=OfficeExtension.ObjectPathType.Method;
...
(function (OfficeExtension) {
	var ObjectPathFactory=(function () {
		...
		ObjectPathFactory.createGlobalObjectObjectPath=function (context) {
			var objectPathInfo={ Id: context._nextId(), ObjectPathType: OfficeExtension.ObjectPathType.GlobalObject, Name: "" };
		...
		ObjectPathFactory.createNewObjectObjectPath=function (context, typeName, isCollection) {
			var objectPathInfo={ Id: context._nextId(), ObjectPathType: OfficeExtension.ObjectPathType.NewObject, Name: typeName };
		...
		ObjectPathFactory.createPropertyObjectPath=function (context, parent, propertyName, isCollection, isInvalidAfterRequest) {
			var objectPathInfo={
				Id: context._nextId(),
				ObjectPathType: OfficeExtension.ObjectPathType.Property,
				Name: propertyName,
		...
		ObjectPathFactory.createIndexerObjectPath=function (context, parent, args) {
			var objectPathInfo={
				Id: context._nextId(),
				ObjectPathType: OfficeExtension.ObjectPathType.Indexer,
				Name: "",
				ParentObjectPathId: parent._objectPath.objectPathInfo.Id,
				ArgumentInfo: {}
			};
		...
		ObjectPathFactory.createMethodObjectPath=function (context, parent, methodName, operationType, args, isCollection, isInvalidAfterRequest, getByIdMethodName) {
			var objectPathInfo={
				Id: context._nextId(),
				ObjectPathType: OfficeExtension.ObjectPathType.Method,
				Name: methodName,
				ParentObjectPathId: parent._objectPath.objectPathInfo.Id,
				ArgumentInfo: {}
			};
			...
			ret.isWriteOperation=(operationType !=OfficeExtension.OperationType.Read);
...
(function (OfficeExtension) {
	var RichApiMessageUtility=(function () {
		...
		RichApiMessageUtility.getResponseBodyFromSafeArray=function (data) {
			var ret=data[OfficeExtension.RichApiResponseMessageIndex.Body];
		...
		RichApiMessageUtility.getResponseHeadersFromSafeArray=function (data) {
			var arrayHeader=data[OfficeExtension.RichApiResponseMessageIndex.Headers];
			if (!arrayHeader) {
				return null;
			}
			var headers={};
		...
		RichApiMessageUtility.getResponseStatusCodeFromSafeArray=function (data) {
			return data[OfficeExtension.RichApiResponseMessageIndex.StatusCode];
		};
...
(function (OfficeExtension) {
	var Utility=(function () {
		...
		Utility.getObjectPathExpression=function (objectPath) {
			...
			while (objectPath) {
				switch (objectPath.objectPathInfo.ObjectPathType) {
					case OfficeExtension.ObjectPathType.GlobalObject:
						ret=ret;
						break;
					case OfficeExtension.ObjectPathType.NewObject:
						ret="new()"+(ret.length > 0 ? "." : "")+ret;
						break;
					case OfficeExtension.ObjectPathType.Method:
						ret=Utility.normalizeName(objectPath.objectPathInfo.Name)+"()"+(ret.length > 0 ? "." : "")+ret;
						break;
					case OfficeExtension.ObjectPathType.Property:
						ret=Utility.normalizeName(objectPath.objectPathInfo.Name)+(ret.length > 0 ? "." : "")+ret;
						break;
					case OfficeExtension.ObjectPathType.Indexer:
						ret="getItem()"+(ret.length > 0 ? "." : "")+ret;
						break;
					case OfficeExtension.ObjectPathType.ReferenceId:
		...
		Utility._buildRequestMessageSafeArray=function (request) {
			var requestFlags=OfficeExtension.ClientRequestFlags.None;
			if (!Utility.isReadonlyRestRequest(request.method)) {
				requestFlags=OfficeExtension.ClientRequestFlags.WriteOperation;
			}
			var requestInfo="";
			if (request.headers) {
...
(function (Word) {
	...
	var Body=(function (_super) {
		...
		Body.prototype.getOoxml=function () {
			var action=_createMethodAction(this.context, this, "GetOoxml", OfficeExtension.OperationType.Read, []);
			var ret=new OfficeExtension.ClientResult();
			_addActionResultHandler(this, action, ret);
			return ret;
		};
		Body.prototype.insertBreak=function (breakType, insertLocation) {
			_createMethodAction(this.context, this, "InsertBreak", OfficeExtension.OperationType.Default, [breakType, insertLocation]);
		};
		Body.prototype.insertContentControl=function () {
			return new Word.ContentControl(this.context, _createMethodObjectPath(this.context, this, "InsertContentControl", OfficeExtension.OperationType.Default, [], false, true));
		};
		Body.prototype.insertFileFromBase64=function (base64File, insertLocation) {
			return new Word.Range(this.context, _createMethodObjectPath(this.context, this, "InsertFileFromBase64", OfficeExtension.OperationType.Default, [base64File, insertLocation], false, true));
		};
		Body.prototype.insertHtml=function (html, insertLocation) {
			return new Word.Range(this.context, _createMethodObjectPath(this.context, this, "InsertHtml", OfficeExtension.OperationType.Default, [html, insertLocation], false, true));
		};
		Body.prototype.insertInlinePictureFromBase64=function (base64EncodedImage, insertLocation) {
			return new Word.InlinePicture(this.context, _createMethodObjectPath(this.context, this, "InsertInlinePictureFromBase64", OfficeExtension.OperationType.Default, [base64EncodedImage, insertLocation], false, true));
		};
		Body.prototype.insertOoxml=function (ooxml, insertLocation) {
			return new Word.Range(this.context, _createMethodObjectPath(this.context, this, "InsertOoxml", OfficeExtension.OperationType.Default, [ooxml, insertLocation], false, true));
		};
		Body.prototype.insertParagraph=function (paragraphText, insertLocation) {
			return new Word.Paragraph(this.context, _createMethodObjectPath(this.context, this, "InsertParagraph", OfficeExtension.OperationType.Default, [paragraphText, insertLocation], false, true));
		};
		Body.prototype.insertText=function (text, insertLocation) {
			return new Word.Range(this.context, _createMethodObjectPath(this.context, this, "InsertText", OfficeExtension.OperationType.Default, [text, insertLocation], false, true));
		};
		Body.prototype.search=function (searchText, searchOptions) {
			searchOptions=_normalizeSearchOptions(this.context, searchOptions);
			return new Word.SearchResultCollection(this.context, _createMethodObjectPath(this.context, this, "Search", OfficeExtension.OperationType.Read, [searchText, searchOptions], true, true));
		};
		Body.prototype.select=function (selectionMode) {
			_createMethodAction(this.context, this, "Select", OfficeExtension.OperationType.Read, [selectionMode]);
		};
	...
	var ContentControl=(function (_super) {
		...
		ContentControl.prototype.delete=function (keepContent) {
			_createMethodAction(this.context, this, "Delete", OfficeExtension.OperationType.Default, [keepContent]);
		};
	...
	var ContentControlCollection=(function (_super) {
		...
		ContentControlCollection.prototype.getById=function (id) {
			return new Word.ContentControl(this.context, _createMethodObjectPath(this.context, this, "GetById", OfficeExtension.OperationType.Read, [id], false, false));
		};
		ContentControlCollection.prototype.getByTag=function (tag) {
			return new Word.ContentControlCollection(this.context, _createMethodObjectPath(this.context, this, "GetByTag", OfficeExtension.OperationType.Read, [tag], true, false));
		};
		ContentControlCollection.prototype.getByTitle=function (title) {
			return new Word.ContentControlCollection(this.context, _createMethodObjectPath(this.context, this, "GetByTitle", OfficeExtension.OperationType.Read, [title], true, false));
		};
	...
	var Document=(function (_super) {
		...
		Document.prototype.getSelection=function () {
			return new Word.Range(this.context, _createMethodObjectPath(this.context, this, "GetSelection", OfficeExtension.OperationType.Read, [], false, true));
		};
		Document.prototype.save=function () {
			_createMethodAction(this.context, this, "Save", OfficeExtension.OperationType.Default, []);
		};
	...
	var InlinePicture=(function (_super) {
		...
		InlinePicture.prototype.getBase64ImageSrc=function () {
			var action=_createMethodAction(this.context, this, "GetBase64ImageSrc", OfficeExtension.OperationType.Read, []);
			var ret=new OfficeExtension.ClientResult();
			_addActionResultHandler(this, action, ret);
			return ret;
		};
	...
	var Section=(function (_super) {
		...
		Section.prototype.getFooter=function (type) {
			return new Word.Body(this.context, _createMethodObjectPath(this.context, this, "GetFooter", OfficeExtension.OperationType.Read, [type], false, true));
		};
		Section.prototype.getHeader=function (type) {
			return new Word.Body(this.context, _createMethodObjectPath(this.context, this, "GetHeader", OfficeExtension.OperationType.Read, [type], false, true));
		};
