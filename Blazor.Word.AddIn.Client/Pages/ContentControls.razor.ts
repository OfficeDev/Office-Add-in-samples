...
export async function insertContentControls() {
  ...
  await Word.run(async (context) => {
    ...
    // Register for the ContentControlAdded event
    currentdocument.onContentControlAdded.add(handleContentControlAdded);

    // Get all paragraphs in the document
    var paragraphs: Word.ParagraphCollection = context.document.body.paragraphs;

    // Load the paragraphs
    paragraphs.load("items"); 
    await context.sync();

    // Iterate through each paragraph and insert a content control
